...
export class FusekiService implements OnModuleInit {
  ...
  async getPOIByUri(params: { uri: string; language?: string }) {
    ...
    // Query để lấy thông tin POI (bao gồm cả rdf:type)
    const query = `
      PREFIX ext: <http://opendatafithou.org/def/extension/>
      PREFIX geo: <http://www.opengis.net/ont/geosparql#>
      PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
    ...
    parameters: {
      ...
      properties: {
        ...
        types: {
          ...
          description:
            'Danh sách loại dịch vụ cần tìm (atm, hospital, school, cafe, bus_stop, playground, restaurant, charging_station, etc.). Để trống để tìm tất cả.',
        },
        includeIoT: {
          type: SchemaType.BOOLEAN,
          description:
            'Bao gồm thông tin trạm cảm biến IoT phủ sóng. Mặc định: false.',
        },
        minAqi: {
          type: SchemaType.NUMBER,
          description:
            'AQI tối thiểu để lọc (0-500). Chỉ trả về địa điểm có aqi >= minAqi.',
        },
        maxAqi: {
          type: SchemaType.NUMBER,
          description:
            'AQI tối đa để lọc (0-500). Dùng maxAqi=50 cho không khí tốt, maxAqi=100 cho không khí trung bình.',
        },
        limit: {
          type: SchemaType.NUMBER,
          description: 'Số POI tối đa trả về (mặc định 150)',
        },
      },
  ...
  async searchNearby(params: {
    ...
    if (!radiusKm || radiusKm <= 0)
      throw new BadRequestException('radiusKm phải > 0');

    // Tăng limit internal nếu có filter AQI để có đủ kết quả sau khi filter
    const hasAqiFilter =
      params.minAqi !== undefined || params.maxAqi !== undefined;
    const internalLimit = hasAqiFilter
      ? 300
      : Math.min(Math.max(params.limit ?? 100, 1), 100);
    ...
    parameters: {
      ...
      properties: {
        ...
        targetType: {
          ...
          description:
            'Loại địa điểm cần tìm (restaurant, cafe, hospital, school, etc.)',
        },
        relatedTypes: {
          type: SchemaType.ARRAY,
          items: { type: SchemaType.STRING },
          description:
            'Danh sách loại địa điểm liên quan (charging_station, parking, bus_stop, atm, etc.). Có thể là 1 hoặc nhiều types.',
        },
        relationship: {
          type: SchemaType.STRING,
          description:
            'Loại quan hệ: "isNextTo" (bên cạnh), "containedInPlace" (trong khu vực), "amenityFeature" (có tiện ích). Mặc định: "isNextTo"',
        },
        minAqi: {
          type: SchemaType.NUMBER,
          description: 'AQI tối thiểu để lọc (0-500)',
        },
        maxAqi: {
          type: SchemaType.NUMBER,
          description:
            'AQI tối đa để lọc. Dùng maxAqi=50 cho không khí tốt, maxAqi=100 cho trung bình.',
        },
        limit: {
          type: SchemaType.NUMBER,
